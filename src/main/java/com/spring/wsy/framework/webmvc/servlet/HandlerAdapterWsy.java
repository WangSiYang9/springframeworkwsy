    ...
    ModelAndViewWsy handle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception{
        ...
        for (int i = 0; i < pa.length ; i ++) {
            for(Annotation a : pa[i]){
                if(a instanceof RequestParamWsy){
                    String paramName = ((RequestParamWsy) a).value();
        ...
        boolean isModelAndView = handlerMapping.getMethod().getReturnType() == ModelAndViewWsy.class;
        if(isModelAndView){
            return (ModelAndViewWsy) result;
        }
        return null;
    }
